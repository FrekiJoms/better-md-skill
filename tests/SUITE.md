...
| 45 | `45-main-readme-creation.md` | Edit | Repo-root README with only a title and one line. Short README (under ~200 lines): NO navigational structure (no Where to Start / Choose Your Path / Navigate tables), NO frontmatter (absent and unrequested), NO badges (no worktree signals stated). Add purpose paragraph, scope callout, features, install, quick start, contributing, license; hero: author an SVG banner or omit it — never fake an image. |
| 46 | `46-non-readme-no-readme-pattern.md` | Improve | A tutorial, not a README: must NOT gain README-only structures — no badge rows, goal-navigation tables, persona sections, or tech-stack icon rows. Fix the heading hierarchy and prose; leave it a tutorial. |
| 47 | `47-mid-quality-readme-to-full.md` | Improve | A main README that is already "polished" but far from the pattern. Short README: no NEW navigational sections added (existing Where to Start / Choose Your Path / Navigate table stay — never removed); NO frontmatter added; NO badges without signals; NO Tech Stack without worktree evidence or tech names in prose. Add scope callout, contributing, license as its own section; fix Install/Quick Start separation and API table. Do NOT stop at prose polish. |
| 48 | `48-worktree-evidence-techstack.md` | Improve | Worktree evidence: `package.json` (react, typescript, vite), `.github/workflows/ci.yml`. Short README: no nav sections, no frontmatter, no badges (no package name or repo URL usable). Expected: Tech Stack icon row via Simple Icons (confirmed slugs `react`, `typescript`, `vite`) right after the opening description; scope callout; separate Install and Quick Start; contributing. |
| 49 | `49-no-worktree-evidence-no-techstack.md` | Improve | Worktree evidence: none; README prose names no technologies. Expected: NO Tech Stack section and no `VISUAL SUGGESTION [TECH_STACK]` (nothing detected to list); report records the omission. No nav sections (short), no frontmatter, no badges. |
| 50 | `50-existing-images-kept-as-is.md` | Improve | README with existing images (`./assets/logo.png`, `./assets/screenshots/dashboard.png`): both must be kept exactly as-is — no SVG authoring to replace them, no conversion, no re-creation. Short README: no nav sections, no frontmatter; badges/tech stack only if detected. |
| 51 | `51-long-readme-full-treatment.md` | Improve | Long README (255 lines). Worktree evidence: `package.json` with `name: "delta-cli"`, `.github/workflows/ci.yml`, `LICENSE`, prose names Node.js/TypeScript. Expected: Where to Start, Choose Your Path, goal-first Navigate table (anchors resolve), Tech Stack via Simple Icons (node.js, typescript), badge row after H1 — npm version badge (`img.shields.io/npm/v/delta-cli`) and license badge constructible; CI badge NOT added (no owner/repo known — no placeholder). |
