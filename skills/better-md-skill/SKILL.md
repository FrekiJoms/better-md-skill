...
The **full README pattern applies only to the main README** (`README.md` at the repository root): frontmatter, badge row, purpose paragraph, scope callout, Where to Start, Choose Your Path, goal-first Navigate table, tech stack, features, install, quick start, API, screenshots, documentation table, contributing, license — with anchor links that resolve to real headings and truthful badges only. Degrade gracefully when the context cannot support parts of it (non-GitHub renderer, unverifiable badges, no assets). **Other document types never receive README-only structures** — no badge rows, goal-navigation tables, persona sections, or tech-stack icon rows in API docs, tutorials, specifications, changelogs, or any non-README document.
